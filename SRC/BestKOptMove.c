...
static GainType BestKOptMoveRec(int k, GainType G0)
{
    ...
    int rest_pts[100];
    int number = 0;
    for (Ntt = t2->CandidateSet; Ntt->To; Ntt++) {
    ...
    for (Nt2 = t2->CandidateSet; (t3 = Nt2->To); Nt2++) {
        ...
        if (t3 == t2->Pred || t3 == t2->Suc ||
            ((G1 = G0 - Ntt->Cost) <= 0 && GainCriterionUsed &&
        ...
        for (X4 = 1; X4 <= 2; X4++) {
            ...
            if (t4 != t1 && !Forbidden(t4, t1) && !Added(t4, t1) &&
                ...
                if (Method == 1 || Method == 2) {
                    Ntt->Value = (1 - Alpha2) * Ntt->Value + Alpha2 * (R + C(t3, t4) - C(t4, t1));
                }
                else {
                    Ntt->Value = G3 - G0 + C(t2, tt2);
                }
